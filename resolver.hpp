...
std::string Unity2Cpp(const std::string& type) {
    static const std::unordered_map<std::string, std::string> typeMap = {
        ...
        {"System.Boolean", "bool"},
        {"System.String", "Resolve::UnityType::String*"},
        {"System.Single", "float"},
        {"System.Double", "double"},
        {"UnityEngine.Vector3", "Resolve::UnityType::Vector3"},
        {"UnityEngine.Vector2", "Resolve::UnityType::Vector2"},
        {"UnityEngine.Vector4", "Resolve::UnityType::Vector4"},
        {"UnityEngine.GameObject", "Resolve::UnityType::GameObject*"},
        {"UnityEngine.Transform", "Resolve::UnityType::Transform*"},
        {"UnityEngine.Animator", "Resolve::UnityType::Animator*"},
        {"UnityEngine.Physics", "Resolve::UnityType::Physics*"},
        {"UnityEngine.Component", "Resolve::UnityType::Component*"},
        {"UnityEngine.Rect", "Resolve::UnityType::Rect"},
        {"UnityEngine.Quaternion", "Resolve::UnityType::Quaternion"},
        {"UnityEngine.Color", "Resolve::UnityType::Color"},
        {"UnityEngine.Matrix4x4", "Resolve::UnityType::Matrix4x4"},
        {"UnityEngine.Rigidbody", "Resolve::UnityType::Rigidbody*"},
        {"System.Void", "void"}
    };
...
public:
	...
	struct Class final {
		...
		std::string getType() {
			...
			std::string object = "";
			if (isEnum) type = "enum";
			if (isInterface) type = "interface";
			if (isAbstract) object = "abstract";
			if (isBlittable) object = "blittable";
			if (isGeneric) object = "generic";
			if (isInflated) object = "inflated";
		...
		[[nodiscard]] auto GetType() -> void* {
			...
			if (mode_ == Mode::Il2Cpp) {
				const auto pUType = Invoke<void*, void*>("il2cpp_class_get_type", address);//DO_API(const Il2CppType*, il2cpp_class_get_type, (Il2CppClass * klass));
		...
		auto FindObjectsByType() -> std::vector<T> {
			...
			if (!pMethod) pMethod = Resolve::Get("UnityEngine.CoreModule.dll")->Get("Object", "UnityEngine")->Get<Method>("FindObjectsOfType", { "System.Type" });
	...
	struct Field final {
		...
		auto GetFieldValue() -> Return* {
			return *(Return **) ((uintptr_t)klass->address + offset);
		}

		template <typename T>
        //void *value;
        //GetStaticValue<void *>(value);
		auto GetStaticValue() const -> T* {
			T* value;
			if (static_field && mode_ == Mode::Il2Cpp){ 
				Invoke<void, void*, T*>("il2cpp_field_static_get_value", address, value);
				return value;
			}else{
				if (static_field){ 
					void* VTable = Invoke<void*>("mono_class_vtable", pDomain, klass->address);
					Invoke<void, void*, void*, T*>("mono_field_static_get_value", VTable, address, value);
					return value;
				}
			}
		}
	...
	public:
		...
		auto Compile() -> void {
			...
			if (address && !function && mode_ == Mode::Mono) function = Resolve::Invoke<void*>("mono_compile_method", address);
		}
		...
		auto RuntimeInvoke(Obj* obj, Args... args) -> Return {
			...
			if (mode_ == Mode::Il2Cpp) {
				if constexpr (std::is_void_v<Return>) {
					Resolve::Invoke<void*>("il2cpp_runtime_invoke", address, obj, sizeof...(Args) ? argArray : nullptr, nullptr);
					return;
				}
			...
			if constexpr (std::is_void_v<Return>) {
				Resolve::Invoke<void*>("mono_runtime_invoke", address, obj, sizeof...(Args) ? argArray : nullptr, nullptr);
				return;
			}
	...
	static std::string BoolToStr(bool type){
		return type ? "true" : "false";
	}

#if WINDOWS_MODE || LINUX_MODE || __ANDROID__ /*__cplusplus >= 202002L*/
	static auto DumpToFile(const std::string path) -> void {
		...
		for (const auto& pAssembly : assembly) {
			for (const auto& pClass : pAssembly->classes) {
				...
				io << std::format(
					"\tAttribut: [Rank: {}] [Flags: {}] [Enum: {}] [Abstract: {}] [Interface: {}] [Blittable: {}] [Generic: {}] [Inflated: {}]\n",
					pClass->rank,
					pClass->flags,
					BoolToStr(pClass->isEnum),
					BoolToStr(pClass->isAbstract),
					BoolToStr(pClass->isInterface),
					BoolToStr(pClass->isBlittable),
					BoolToStr(pClass->isGeneric),
					BoolToStr(pClass->isInflated)
				);
				io << std::format("\t{} {}{} ", pClass->getType(), pClass->name, pClass->parent.empty() ? "" : " : " + pClass->parent);
				io << "{\n\n";
				...
				for (const auto& pMethod : pClass->methods) {
					...
					io << std::format("\t\t//{}{} {}(", pMethod->static_function ? "static " : "", Unity2Cpp(pMethod->return_type->name), pMethod->name);//System.Void
					std::string params2{};
					for (const auto& pArg : pMethod->args){
						params2 += std::format("{} {}, ", Unity2Cpp(pArg->pType->name), pArg->name);
					}
					if (!params2.empty()) {
						params2.pop_back();
						params2.pop_back();
					}
					io << (params2.empty() ? "" : params2) << ");\n\n";
		...
		for (const auto& pAssembly : assembly) {
			for (const auto& pClass : pAssembly->classes) {
				...
				if (field->type->name == "System.String") {
					io2 << std::format("\t\tResolve::UnityType::String* {};\n", name);
				...
				if (field->type->name == "UnityEngine.Vector3") {
					io2 << std::format("\t\tResolve::UnityType::Vector3 {};\n", name);
				...
				if (field->type->name == "UnityEngine.Vector2") {
					io2 << std::format("\t\tResolve::UnityType::Vector2 {};\n", name);
				...
				if (field->type->name == "UnityEngine.Vector4") {
					io2 << std::format("\t\tResolve::UnityType::Vector4 {};\n", name);
				...
				if (field->type->name == "UnityEngine.GameObject") {
					io2 << std::format("\t\tResolve::UnityType::GameObject* {};\n", name);
					if (!pClass->fields[i + 1]->static_field && (pClass->fields[i + 1]->offset - field->offset) > sizeof(void*)) io2 << std::format("\t\tchar {}_[0x{:06X}];\n", name, pClass->fields[i + 1]->offset - field->offset - sizeof(void*));
					continue;
				}

				if (field->type->name == "UnityEngine.Transform") {
					io2 << std::format("\t\tResolve::UnityType::Transform* {};\n", name);
					if (!pClass->fields[i + 1]->static_field && (pClass->fields[i + 1]->offset - field->offset) > sizeof(void*)) io2 << std::format("\t\tchar {}_[0x{:06X}];\n", name, pClass->fields[i + 1]->offset - field->offset - sizeof(void*));
					continue;
				}

				if (field->type->name == "UnityEngine.Animator") {
					io2 << std::format("\t\tResolve::UnityType::Animator* {};\n", name);
					if (!pClass->fields[i + 1]->static_field && (pClass->fields[i + 1]->offset - field->offset) > sizeof(void*)) io2 << std::format("\t\tchar {}_[0x{:06X}];\n", name, pClass->fields[i + 1]->offset - field->offset - sizeof(void*));
					continue;
				}

				if (field->type->name == "UnityEngine.Physics") {
					io2 << std::format("\t\tResolve::UnityType::Physics* {};\n", name);
					if (!pClass->fields[i + 1]->static_field && (pClass->fields[i + 1]->offset - field->offset) > sizeof(void*)) io2 << std::format("\t\tchar {}_[0x{:06X}];\n", name, pClass->fields[i + 1]->offset - field->offset - sizeof(void*));
					continue;
				}

				if (field->type->name == "UnityEngine.Component") {
					io2 << std::format("\t\tResolve::UnityType::Component* {};\n", name);
					if (!pClass->fields[i + 1]->static_field && (pClass->fields[i + 1]->offset - field->offset) > sizeof(void*)) io2 << std::format("\t\tchar {}_[0x{:06X}];\n", name, pClass->fields[i + 1]->offset - field->offset - sizeof(void*));
					continue;
				}

				if (field->type->name == "UnityEngine.Rect") {
					io2 << std::format("\t\tResolve::UnityType::Rect {};\n", name);
				...
				if (field->type->name == "UnityEngine.Quaternion") {
					io2 << std::format("\t\tResolve::UnityType::Quaternion {};\n", name);
				...
				if (field->type->name == "UnityEngine.Color") {
					io2 << std::format("\t\tResolve::UnityType::Color {};\n", name);
				...
				if (field->type->name == "UnityEngine.Matrix4x4") {
					io2 << std::format("\t\tResolve::UnityType::Matrix4x4 {};\n", name);
				...
				if (field->type->name == "UnityEngine.Rigidbody") {
					io2 << std::format("\t\tResolve::UnityType::Rigidbody* {};\n", name);
					if (!pClass->fields[i + 1]->static_field && (pClass->fields[i + 1]->offset - field->offset) > sizeof(void*)) io2 << std::format("\t\tchar {}_[0x{:06X}];\n", name, pClass->fields[i + 1]->offset - field->offset - sizeof(void*));
					continue;
				}
	...
	static auto ForeachAssembly() -> void {
		...
		if (mode_ == Mode::Il2Cpp) {
			...
			for (auto i = 0; i < nrofassemblies; i++) {
				...
				Resolve::assembly.push_back(assembly);
	...
	static auto ForeachClass(Assembly* assembly, void* image) -> void {
		...
		if (mode_ == Mode::Il2Cpp) {
			...
			for (auto i = 0; i < count; i++) {
				...
				pAClass->isEnum = Invoke<bool>("il2cpp_class_is_enum", pClass);
				pAClass->isGeneric = Invoke<bool>("il2cpp_class_is_generic", pClass);
				pAClass->isInflated = Invoke<bool>("il2cpp_class_is_inflated", pClass);
				pAClass->isInterface = Invoke<bool>("il2cpp_class_is_interface", pClass);
				pAClass->isAbstract = Invoke<bool>("il2cpp_class_is_abstract", pClass);
				pAClass->isBlittable = Invoke<bool>("il2cpp_class_is_blittable", pClass);
				pAClass->flags = Invoke<int>("il2cpp_class_get_flags", pClass);
				pAClass->rank = Invoke<int>("il2cpp_class_get_rank", pClass);
	...
	public:
		...
		struct Vector3 {
			...
			auto ToVectors(Vector3* m_pForward, Vector3* m_pRight, Vector3* m_pUp) const -> void {
				constexpr auto m_fDeg2Rad = PI / 180.F;
		...
		struct Quaternion {
			...
			[[nodiscard]] auto ToEuler() const -> Vector3 {
				...
				constexpr auto m_fRad2Deg = 180.F / PI;
		...
		struct Object {
			...
			auto GetType() -> CsType* {
				...
				static Method* method;
				if (!method) method = Get("mscorlib.dll")->Get("Object", "UnityEngine", "System")->Get<Method>("GetType");
			...
			auto ToString() -> String* {
				...
				static Method* method;
				if (!method) method = Get("mscorlib.dll")->Get("Object", "UnityEngine", "System")->Get<Method>("ToString");
			...
			int GetHashCode() {
				...
				static Method* method;
				if (!method) method = Get("mscorlib.dll")->Get("Object", "UnityEngine", "System")->Get<Method>("GetHashCode");
		...
		struct String : Object {
			...
			static auto New(const std::string& str) -> String* {
				if (mode_ == Mode::Il2Cpp) return Resolve::Invoke<String*, const char*>("il2cpp_string_new", str.c_str());
				return Resolve::Invoke<String*, void*, const char*>("mono_string_new", Resolve::Invoke<void*>("mono_get_root_domain"), str.c_str());
			}
		};
		...
		struct Array : Object {
			...
			static auto New(const Class* kalss, const std::uintptr_t size) -> Array* {
				if (mode_ == Mode::Il2Cpp) return Resolve::Invoke<Array*, void*, std::uintptr_t>("il2cpp_array_new", kalss->address, size);
				return Resolve::Invoke<Array*, void*, void*, std::uintptr_t>("mono_array_new", pDomain, kalss->address, size);
			}
		};
		...
		struct UnityObject : Object {
			...
			auto GetName() -> String* {
				...
				static Method* method;
				if (!method) method = Get("UnityEngine.CoreModule.dll")->Get("Object", "UnityEngine")->Get<Method>("get_name");
				if (method) return method->Invoke<String*>(this);
				return {};
			}
			...
			auto ToString() -> String* {
				...
				static Method* method;
				if (!method) method = Get("UnityEngine.CoreModule.dll")->Get("Object", "UnityEngine")->Get<Method>("ToString");
				if (method) return method->Invoke<String*>(this);
				return {};
			}
			...
			static auto Instantiate(UnityObject* original) -> UnityObject* {
				...
				static Method* method;
				if (!method) method = Get("UnityEngine.CoreModule.dll")->Get("Object", "UnityEngine")->Get<Method>("Instantiate", { "*" });
			...
			static auto Destroy(UnityObject* original) -> void {
				if (!original) return;
				static Method* method;
				if (!method) method = Get("UnityEngine.CoreModule.dll")->Get("Object", "UnityEngine")->Get<Method>("Destroy", { "*" });
		...
		struct Component : public UnityObject {
			auto GetTransform() -> Transform* {
				...
				static Method* method;
				if (!method) method = Get("UnityEngine.CoreModule.dll")->Get("Component", "UnityEngine")->Get<Method>("get_transform");
			...
			auto GetGameObject() -> GameObject* {
				...
				static Method* method;
				if (!method) method = Get("UnityEngine.CoreModule.dll")->Get("Component", "UnityEngine")->Get<Method>("get_gameObject");
			...
			auto GetTag() -> String* {
				...
				static Method* method;
				if (!method) method = Get("UnityEngine.CoreModule.dll")->Get("Component", "UnityEngine")->Get<Method>("get_tag");
				if (method) return method->Invoke<String*>(this);
				return {};
			}
			...
			auto GetComponentsInChildren() -> std::vector<T> {
				...
				static Method* method;
				if (!method) method = Get("UnityEngine.CoreModule.dll")->Get("Component", "UnityEngine")->Get<Method>("GetComponentsInChildren");
			...
			auto GetComponentsInChildren(Class* pClass) -> std::vector<T> {
				...
				if (!method) method = Get("UnityEngine.CoreModule.dll")->Get("Component", "UnityEngine")->Get<Method>("GetComponentsInChildren", { "System.Type" });
			...
			auto GetComponents() -> std::vector<T> {
				static Method* method;
				if (!method) method = Get("UnityEngine.CoreModule.dll")->Get("Component", "UnityEngine")->Get<Method>("GetComponents");
				if (method) return method->Invoke<Array<T>*>(this)->ToVector();
				return {};
			}
			...
			auto GetComponents(Class* pClass) -> std::vector<T> {
				...
				if (!method || !obj) {
					method = Get("UnityEngine.CoreModule.dll")->Get("Component", "UnityEngine")->Get<Method>("GetComponents", { "System.Type" });
					obj = pClass->GetType();
				}
			...
			auto GetComponentsInParent() -> std::vector<T> {
				static Method* method;
				if (!method) method = Get("UnityEngine.CoreModule.dll")->Get("Component", "UnityEngine")->Get<Method>("GetComponentsInParent");
				if (method) return method->Invoke<Array<T>*>(this)->ToVector();
				return {};
			}
			...
			auto GetComponentsInParent(Class* pClass) -> std::vector<T> {
				...
				if (!method || !obj) {
					method = Get("UnityEngine.CoreModule.dll")->Get("Component", "UnityEngine")->Get<Method>("GetComponentsInParent", { "System.Type" });
					obj = pClass->GetType();
				}
				if (method) return method->Invoke<Array<T>*>(this, obj)->ToVector();
				return {};
			...
			auto GetComponentInChildren(Class* pClass) -> T {
				static Method* method;
				if (!method) method = Get("UnityEngine.CoreModule.dll")->Get("Component", "UnityEngine")->Get<Method>("GetComponentInChildren", { "System.Type" });
			...
			auto GetComponentInParent(Class* pClass) -> T {
				static Method* method;
				if (!method) method = Get("UnityEngine.CoreModule.dll")->Get("Component", "UnityEngine")->Get<Method>("GetComponentInParent", { "System.Type" });
				if (method) return method->Invoke<T>(this, pClass->GetType());
				return T();
			}
		};
		...
		struct Camera : Component {
			...
			static auto GetMain() -> Camera* {
				static Method* method;
				if (!method) method = Get("UnityEngine.CoreModule.dll")->Get("Camera", "UnityEngine")->Get<Method>("get_main");
			...
			static auto GetCurrent() -> Camera* {
				static Method* method;
				if (!method) method = Get("UnityEngine.CoreModule.dll")->Get("Camera", "UnityEngine")->Get<Method>("get_current");
				if (method) return method->Invoke<Camera*>();
				return nullptr;
			}
			...
			static auto GetAllCount() -> int {
				static Method* method;
				if (!method) method = Get("UnityEngine.CoreModule.dll")->Get("Camera", "UnityEngine")->Get<Method>("get_allCamerasCount");
			...
			static auto GetAllCamera() -> std::vector<Camera*> {
				...
				if (!method || !klass) {
					method = Get("UnityEngine.CoreModule.dll")->Get("Camera", "UnityEngine")->Get<Method>("GetAllCameras", { "*" });
					klass = Get("UnityEngine.CoreModule.dll")->Get("Camera", "UnityEngine");
				}
			...
			auto GetDepth() -> float {
				...
				static Method* method;
				if (!method) method = Get("UnityEngine.CoreModule.dll")->Get("Camera", "UnityEngine")->Get<Method>("get_depth");
			...
			auto SetDepth(const float depth) -> void {
				...
				static Method* method;
				if (!method) method = Get("UnityEngine.CoreModule.dll")->Get("Camera", "UnityEngine")->Get<Method>("set_depth", { "*" });
			...
			auto SetFoV(const float fov) -> void {
				...
				if (!method_fieldOfView) method_fieldOfView = Get("UnityEngine.CoreModule.dll")->Get("Camera", "UnityEngine")->Get<Method>("set_fieldOfView", { "*" });
			...
			auto GetFoV() -> float {
				...
				static Method* method_fieldOfView;
				if (!method_fieldOfView) method_fieldOfView = Get("UnityEngine.CoreModule.dll")->Get("Camera", "UnityEngine")->Get<Method>("get_fieldOfView");
			...
			auto WorldToScreenPoint(const Vector3& position, const Eye eye = Eye::Mono) -> Vector3 {
				...
				static Method* method;
				if (!method) {
					if (mode_ == Mode::Mono) method = Get("UnityEngine.CoreModule.dll")->Get("Camera", "UnityEngine")->Get<Method>("WorldToScreenPoint_Injected");
					else method = Get("UnityEngine.CoreModule.dll")->Get("Camera", "UnityEngine")->Get<Method>("WorldToScreenPoint", { "*", "*" });
				}
			...
			auto ScreenToWorldPoint(const Vector3& position, const Eye eye = Eye::Mono) -> Vector3 {
				...
				static Method* method;
				if (!method) method = Get("UnityEngine.CoreModule.dll")->Get("Camera", "UnityEngine")->Get<Method>(mode_ == Mode::Mono ? "ScreenToWorldPoint_Injected" : "ScreenToWorldPoint");
				if (mode_ == Mode::Mono && method) {
					const Vector3 vec3{};
					method->Invoke<void>(this, position, eye, &vec3);
					return vec3;
			...
			auto CameraToWorldMatrix() -> Matrix4x4 {
				...
				static Method* method;
				if (!method) method = Get("UnityEngine.CoreModule.dll")->Get("Camera", "UnityEngine")->Get<Method>(mode_ == Mode::Mono ? "get_cameraToWorldMatrix_Injected" : "get_cameraToWorldMatrix");
				if (mode_ == Mode::Mono && method) {
					Matrix4x4 matrix4{};
		...
		struct Transform : Component {
			auto GetPosition() -> Vector3 {
				...
				if (!method) method = Get("UnityEngine.CoreModule.dll")->Get("Transform", "UnityEngine")->Get<Method>("get_position_Injected");
				const Vector3 vec3{};
			...
			auto SetPosition(const Vector3& position) -> void {
				...
				if (!method) method = Get("UnityEngine.CoreModule.dll")->Get("Transform", "UnityEngine")->Get<Method>("set_position_Injected");
			...
			auto GetRight() -> Vector3 {
				...
				if (!method) method = Get("UnityEngine.CoreModule.dll")->Get("Transform", "UnityEngine")->Get<Method>("get_right");
			...
			auto SetRight(const Vector3& value) -> void {
				...
				if (!method) method = Get("UnityEngine.CoreModule.dll")->Get("Transform", "UnityEngine")->Get<Method>("set_right");
			...
			auto GetUp() -> Vector3 {
				...
				if (!method) method = Get("UnityEngine.CoreModule.dll")->Get("Transform", "UnityEngine")->Get<Method>("get_up");
				if (method) return method->Invoke<Vector3>(this);
				return {};
			}
			...
			auto SetUp(const Vector3& value) -> void {
				...
				if (!method) method = Get("UnityEngine.CoreModule.dll")->Get("Transform", "UnityEngine")->Get<Method>("set_up");
				if (method) return method->Invoke<void>(this, value);
			}
			...
			auto GetForward() -> Vector3 {
				...
				if (!method) method = Get("UnityEngine.CoreModule.dll")->Get("Transform", "UnityEngine")->Get<Method>("get_forward");
				if (method) return method->Invoke<Vector3>(this);
				return {};
			}
			...
			auto SetForward(const Vector3& value) -> void {
				...
				if (!method) method = Get("UnityEngine.CoreModule.dll")->Get("Transform", "UnityEngine")->Get<Method>("set_forward");
				if (method) return method->Invoke<void>(this, value);
			}
			...
			auto GetRotation() -> Quaternion {
				...
				if (!method) method = Get("UnityEngine.CoreModule.dll")->Get("Transform", "UnityEngine")->Get<Method>(mode_ == Mode::Mono ? "get_rotation_Injected" : "get_rotation");
				if (mode_ == Mode::Mono && method) {
			...
			auto SetRotation(const Quaternion& position) -> void {
				...
				if (!method) method = Get("UnityEngine.CoreModule.dll")->Get("Transform", "UnityEngine")->Get<Method>(mode_ == Mode::Mono ? "set_rotation_Injected" : "set_rotation");
			...
			auto GetLocalPosition() -> Vector3 {
				...
				if (!method) method = Get("UnityEngine.CoreModule.dll")->Get("Transform", "UnityEngine")->Get<Method>(mode_ == Mode::Mono ? "get_localPosition_Injected" : "get_localPosition");
				if (mode_ == Mode::Mono && method) {
					const Vector3 vec3{};
					method->Invoke<void>(this, &vec3);
					return vec3;
			...
			auto SetLocalPosition(const Vector3& position) -> void {
				...
				if (!method) method = Get("UnityEngine.CoreModule.dll")->Get("Transform", "UnityEngine")->Get<Method>(mode_ == Mode::Mono ? "set_localPosition_Injected" : "set_localPosition");
				if (mode_ == Mode::Mono && method) return method->Invoke<void>(this, &position);
				if (method) return method->Invoke<void>(this, position);
			}
			...
			auto GetLocalRotation() -> Quaternion {
				...
				if (!method) method = Get("UnityEngine.CoreModule.dll")->Get("Transform", "UnityEngine")->Get<Method>(mode_ == Mode::Mono ? "get_localRotation_Injected" : "get_localRotation");
				if (mode_ == Mode::Mono && method) {
					const Quaternion vec3{};
					method->Invoke<void>(this, &vec3);
					return vec3;
			...
			auto SetLocalRotation(const Quaternion& position) -> void {
				...
				if (!method) method = Get("UnityEngine.CoreModule.dll")->Get("Transform", "UnityEngine")->Get<Method>(mode_ == Mode::Mono ? "set_localRotation_Injected" : "set_localRotation");
				if (mode_ == Mode::Mono && method) return method->Invoke<void>(this, &position);
				if (method) return method->Invoke<void>(this, position);
			}
			...
			auto GetLocalScale() -> Vector3 {
				...
				if (!method) method = Get("UnityEngine.CoreModule.dll")->Get("Transform", "UnityEngine")->Get<Method>("get_localScale_Injected");
				const Vector3 vec3{};
				method->Invoke<void>(this, &vec3);
				return vec3;
			}
			...
			auto SetLocalScale(const Vector3& position) -> void {
				...
				if (!method) method = Get("UnityEngine.CoreModule.dll")->Get("Transform", "UnityEngine")->Get<Method>(mode_ == Mode::Mono ? "set_localScale_Injected" : "set_localScale");
				if (mode_ == Mode::Mono && method) return method->Invoke<void>(this, &position);
				if (method) return method->Invoke<void>(this, position);
			}
			...
			auto GetChildCount() -> int {
				...
				if (!method) method = Get("UnityEngine.CoreModule.dll")->Get("Transform", "UnityEngine")->Get<Method>("get_childCount");
				if (method) return method->Invoke<int>(this);
				return 0;
			}
			...
			auto GetChild(const int index) -> Transform* {
				...
				if (!method) method = Get("UnityEngine.CoreModule.dll")->Get("Transform", "UnityEngine")->Get<Method>("GetChild");
			...
			auto GetRoot() -> Transform* {
				...
				if (!method) method = Get("UnityEngine.CoreModule.dll")->Get("Transform", "UnityEngine")->Get<Method>("GetRoot");
				if (method) return method->Invoke<Transform*>(this);
				return nullptr;
			}
			...
			auto GetParent() -> Transform* {
				...
				if (!method) method = Get("UnityEngine.CoreModule.dll")->Get("Transform", "UnityEngine")->Get<Method>("GetParent");
				if (method) return method->Invoke<Transform*>(this);
				return nullptr;
			}
			...
			auto GetLossyScale() -> Vector3 {
				...
				if (!method) method = Get("UnityEngine.CoreModule.dll")->Get("Transform", "UnityEngine")->Get<Method>(mode_ == Mode::Mono ? "get_lossyScale_Injected" : "get_lossyScale");
				if (mode_ == Mode::Mono && method) {
					const Vector3 vec3{};
					method->Invoke<void>(this, &vec3);
					return vec3;
			...
			auto TransformPoint(const Vector3& position) -> Vector3 {
				...
				if (!method) method = Get("UnityEngine.CoreModule.dll")->Get("Transform", "UnityEngine")->Get<Method>(mode_ == Mode::Mono ? "TransformPoint_Injected" : "TransformPoint");
				if (mode_ == Mode::Mono && method) {
					const Vector3 vec3{};
			...
			auto LookAt(const Vector3& worldPosition) -> void {
				...
				if (!method) method = Get("UnityEngine.CoreModule.dll")->Get("Transform", "UnityEngine")->Get<Method>("LookAt", { "Vector3" });
			...
			auto Rotate(const Vector3& eulers) -> void {
				...
				if (!method) method = Get("UnityEngine.CoreModule.dll")->Get("Transform", "UnityEngine")->Get<Method>("Rotate", { "Vector3" });
		...
		struct GameObject : UnityObject {
			static auto Create(GameObject* obj, const std::string& name) -> void {
				if (!obj) return;
				static Method* method;
				if (!method) method = Get("UnityEngine.CoreModule.dll")->Get("GameObject", "UnityEngine")->Get<Method>("Internal_CreateGameObject");
			...
			static auto FindGameObjectsWithTag(const std::string& name) -> std::vector<GameObject*> {
				static Method* method;
				if (!method) method = Get("UnityEngine.CoreModule.dll")->Get("GameObject", "UnityEngine")->Get<Method>("FindGameObjectsWithTag");
				if (method) {
			...
			static auto Find(const std::string& name) -> GameObject* {
				static Method* method;
				if (!method) method = Get("UnityEngine.CoreModule.dll")->Get("GameObject", "UnityEngine")->Get<Method>("Find");
			...
			auto GetActive() -> bool {
				...
				if (!method) method = Get("UnityEngine.CoreModule.dll")->Get("GameObject", "UnityEngine")->Get<Method>("get_active");
				if (method) return method->Invoke<bool>(this);
				return false;
			}
			...
			auto SetActive(const bool value) -> void {
				...
				if (!method) method = Get("UnityEngine.CoreModule.dll")->Get("GameObject", "UnityEngine")->Get<Method>("set_active");
				if (method) return method->Invoke<void>(this, value);
			}
			...
			auto GetActiveSelf() -> bool {
				...
				if (!method) method = Get("UnityEngine.CoreModule.dll")->Get("GameObject", "UnityEngine")->Get<Method>("get_activeSelf");
				if (method) return method->Invoke<bool>(this);
				return false;
			}
			...
			auto GetActiveInHierarchy() -> bool {
				...
				if (!method) method = Get("UnityEngine.CoreModule.dll")->Get("GameObject", "UnityEngine")->Get<Method>("get_activeInHierarchy");
				if (method) return method->Invoke<bool>(this);
				return false;
			}
			...
			auto GetIsStatic() -> bool {
				...
				if (!method) method = Get("UnityEngine.CoreModule.dll")->Get("GameObject", "UnityEngine")->Get<Method>("get_isStatic");
				if (method) return method->Invoke<bool>(this);
				return false;
			}
			...
			auto GetTransform() -> Transform* {
				...
				if (!method) method = Get("UnityEngine.CoreModule.dll")->Get("GameObject", "UnityEngine")->Get<Method>("get_transform");
				if (method) return method->Invoke<Transform*>(this);
				return nullptr;
			}
			...
			auto GetTag() -> String* {
				...
				static Method* method;
				if (!method) method = Get("UnityEngine.CoreModule.dll")->Get("GameObject", "UnityEngine")->Get<Method>("get_tag");
				if (method) return method->Invoke<String*>(this);
				return {};
			}
			...
			auto GetComponent() -> T {
				...
				static Method* method;
				if (!method) method = Get("UnityEngine.CoreModule.dll")->Get("GameObject", "UnityEngine")->Get<Method>("GetComponent");
			...
			auto GetComponent(Class* type) -> T {
				...
				static Method* method;
				if (!method) method = Get("UnityEngine.CoreModule.dll")->Get("GameObject", "UnityEngine")->Get<Method>("GetComponent", { "System.Type" });
			...
			auto GetComponentInChildren(Class* type) -> T {
				...
				static Method* method;
				if (!method) method = Get("UnityEngine.CoreModule.dll")->Get("GameObject", "UnityEngine")->Get<Method>("GetComponentInChildren", { "System.Type" });
				if (method) return method->Invoke<T>(this, type->GetType());
				return T();
			}
			...
			auto GetComponentInParent(Class* type) -> T {
				...
				static Method* method;
				if (!method) method = Get("UnityEngine.CoreModule.dll")->Get("GameObject", "UnityEngine")->Get<Method>("GetComponentInParent", { "System.Type" });
				if (method) return method->Invoke<T>(this, type->GetType());
				return T();
			}
			...
			auto GetComponents(Class* type, bool useSearchTypeAsArrayReturnType = false, bool recursive = false, bool includeInactive = true, bool reverse = false, List<T>* resultList = nullptr) -> std::vector<T> {
				...
				static Method* method;
				if (!method) method = Get("UnityEngine.CoreModule.dll")->Get("GameObject", "UnityEngine")->Get<Method>("GetComponentsInternal");
		...
		struct LayerMask : Object {
			...
			static auto NameToLayer(const std::string& layerName) -> int {
				static Method* method;
				if (!method) method = Get("UnityEngine.CoreModule.dll")->Get("LayerMask", "UnityEngine")->Get<Method>("NameToLayer");
			...
			static auto LayerToName(const int layer) -> String* {
				static Method* method;
				if (!method) method = Get("UnityEngine.CoreModule.dll")->Get("LayerMask", "UnityEngine")->Get<Method>("LayerToName");
		...
		struct Rigidbody : Component {
			auto GetDetectCollisions() -> bool {
				static Method* method;
				if (!method) method = Get("UnityEngine.PhysicsModule.dll")->Get("Rigidbody", "UnityEngine")->Get<Method>("get_detectCollisions");
				if (method) return method->Invoke<bool>(this);
				return false;
			}
			...
			auto SetDetectCollisions(const bool value) -> void {
				static Method* method;
				if (!method) method = Get("UnityEngine.PhysicsModule.dll")->Get("Rigidbody", "UnityEngine")->Get<Method>("set_detectCollisions");
				if (method) return method->Invoke<void>(this, value);
			}

			auto GetVelocity() -> Vector3 {
				static Method* method;
				if (!method) method = Get("UnityEngine.PhysicsModule.dll")->Get("Rigidbody", "UnityEngine")->Get<Method>(mode_ == Mode::Mono ? "get_velocity_Injected" : "get_velocity");
				if (mode_ == Mode::Mono && method) {
					Vector3 vector;
			...
			auto SetVelocity(Vector3 value) -> void {
				static Method* method;
				if (!method) method = Get("UnityEngine.PhysicsModule.dll")->Get("Rigidbody", "UnityEngine")->Get<Method>(mode_ == Mode::Mono ? "set_velocity_Injected" : "set_velocity");
		...
		struct Collider : Component {
			auto GetBounds() -> Bounds {
				...
				static Method* method;
				if (!method) method = Get("UnityEngine.PhysicsModule.dll")->Get("Collider", "UnityEngine")->Get<Method>("get_bounds_Injected");
				if (method) {
					Bounds bounds;
		...
		struct Mesh : UnityObject {
			auto GetBounds() -> Bounds {
				...
				static Method* method;
				if (!method) method = Get("UnityEngine.CoreModule.dll")->Get("Mesh", "UnityEngine")->Get<Method>("get_bounds_Injected");
				if (method) {
					Bounds bounds;
					method->Invoke<void>(this, &bounds);
					return bounds;
		...
		struct CapsuleCollider : Collider {
			auto GetCenter() -> Vector3 {
				static Method* method;
				if (!method) method = Get("UnityEngine.PhysicsModule.dll")->Get("CapsuleCollider", "UnityEngine")->Get<Method>("get_center");
				if (method) return method->Invoke<Vector3>(this);
				return {};
			}
			...
			auto GetDirection() -> Vector3 {
				static Method* method;
				if (!method) method = Get("UnityEngine.PhysicsModule.dll")->Get("CapsuleCollider", "UnityEngine")->Get<Method>("get_direction");
				if (method) return method->Invoke<Vector3>(this);
				return {};
			}
			...
			auto GetHeightn() -> Vector3 {
				static Method* method;
				if (!method) method = Get("UnityEngine.PhysicsModule.dll")->Get("CapsuleCollider", "UnityEngine")->Get<Method>("get_height");
				if (method) return method->Invoke<Vector3>(this);
				return {};
			}
			...
			auto GetRadius() -> Vector3 {
				static Method* method;
				if (!method) method = Get("UnityEngine.PhysicsModule.dll")->Get("CapsuleCollider", "UnityEngine")->Get<Method>("get_radius");
				if (method) return method->Invoke<Vector3>(this);
				return {};
			}
		};
		...
		struct BoxCollider : Collider {
			auto GetCenter() -> Vector3 {
				static Method* method;
				if (!method) method = Get("UnityEngine.PhysicsModule.dll")->Get("BoxCollider", "UnityEngine")->Get<Method>("get_center");
				if (method) return method->Invoke<Vector3>(this);
				return {};
			}
			...
			auto GetSize() -> Vector3 {
				static Method* method;
				if (!method) method = Get("UnityEngine.PhysicsModule.dll")->Get("BoxCollider", "UnityEngine")->Get<Method>("get_size");
				if (method) return method->Invoke<Vector3>(this);
				return {};
			}
		};
		...
		struct Renderer : Component {
			auto GetBounds() -> Bounds {
				...
				static Method* method;
				if (!method) method = Get("UnityEngine.CoreModule.dll")->Get("Renderer", "UnityEngine")->Get<Method>("get_bounds_Injected");
				if (method) {
					Bounds bounds;
					method->Invoke<void>(this, &bounds);
					return bounds;
		...
		struct Behaviour : public Component {
			auto GetEnabled() -> bool {
				...
				static Method* method;
				if (!method) method = Get("UnityEngine.CoreModule.dll")->Get("Behaviour", "UnityEngine")->Get<Method>("get_enabled");
				if (method) return method->Invoke<bool>(this);
				return false;
			}
			...
			auto SetEnabled(const bool value) -> void {
				...
				static Method* method;
				if (!method) method = Get("UnityEngine.CoreModule.dll")->Get("Behaviour", "UnityEngine")->Get<Method>("set_enabled");
				if (method) return method->Invoke<void>(this, value);
			}
		};
		...
		struct Physics : Object {
			static auto Linecast(const Vector3& start, const Vector3& end) -> bool {
				static Method* method;
				if (!method) method = Get("UnityEngine.PhysicsModule.dll")->Get("Physics", "UnityEngine")->Get<Method>("Linecast", { "*", "*" });
			...
			static auto Raycast(const Vector3& origin, const Vector3& direction, const float maxDistance) -> bool {
				static Method* method;
				if (!method) method = Get("UnityEngine.PhysicsModule.dll")->Get("Physics", "UnityEngine")->Get<Method>("Raycast", { "UnityEngine.Vector3", "UnityEngine.Vector3", "System.Single" });
			...
			static auto Raycast(const Ray& origin, const RaycastHit* direction, const float maxDistance) -> bool {
				static Method* method;
				if (!method) method = Get("UnityEngine.PhysicsModule.dll")->Get("Physics", "UnityEngine")->Get<Method>("Raycast", { "UnityEngine.Ray", "UnityEngine.RaycastHit&", "System.Single" });
			...
			static auto IgnoreCollision(Collider* collider1, Collider* collider2) -> void {
				static Method* method;
				if (!method) method = Get("UnityEngine.PhysicsModule.dll")->Get("Physics", "UnityEngine")->Get<Method>("IgnoreCollision1", { "*", "*" });
		...
		struct Animator : Behaviour {
			...
			auto GetBoneTransform(const HumanBodyBones humanBoneId) -> Transform* {
				...
				if (!method) method = Get("UnityEngine.AnimationModule.dll")->Get("Animator", "UnityEngine")->Get<Method>("GetBoneTransform");
		...
		struct Time {
			static auto GetTime() -> float {
				static Method* method;
				if (!method) method = Get("UnityEngine.CoreModule.dll")->Get("Time", "UnityEngine")->Get<Method>("get_time");
			...
			static auto GetDeltaTime() -> float {
				static Method* method;
				if (!method) method = Get("UnityEngine.CoreModule.dll")->Get("Time", "UnityEngine")->Get<Method>("get_deltaTime");
				if (method) return method->Invoke<float>();
				return 0.0f;
			}
			...
			static auto GetFixedDeltaTime() -> float {
				static Method* method;
				if (!method) method = Get("UnityEngine.CoreModule.dll")->Get("Time", "UnityEngine")->Get<Method>("get_fixedDeltaTime");
				if (method) return method->Invoke<float>();
				return 0.0f;
			}
			...
			static auto GetTimeScale() -> float {
				static Method* method;
				if (!method) method = Get("UnityEngine.CoreModule.dll")->Get("Time", "UnityEngine")->Get<Method>("get_timeScale");
				if (method) return method->Invoke<float>();
				return 0.0f;
			}
			...
			static auto SetTimeScale(const float value) -> void {
				static Method* method;
				if (!method) method = Get("UnityEngine.CoreModule.dll")->Get("Time", "UnityEngine")->Get<Method>("set_timeScale");
		...
		struct Screen {
			static auto get_width() -> Int32 {
				static Method* method;
				if (!method) method = Get("UnityEngine.CoreModule.dll")->Get("Screen", "UnityEngine")->Get<Method>("get_width");
			...
			static auto get_height() -> Int32 {
				static Method* method;
				if (!method) method = Get("UnityEngine.CoreModule.dll")->Get("Screen", "UnityEngine")->Get<Method>("get_height");
				if (method) return method->Invoke<int32_t>();
				return 0;
			}

            static auto SetResolution(int width, int height, bool fullscreen) -> void {
                static Method* pointer;
                if(!pointer) pointer = Get("UnityEngine.CoreModule.dll")->Get("Screen", "UnityEngine")->Get<Method>("SetResolution");
                if(pointer) return pointer->Invoke<void>(width, height, fullscreen);
            }
		};
